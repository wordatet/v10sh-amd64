...
int
execute(struct trenod *argt, int exec_link, int errorflg, int *pf1, int *pf2)
{
	/*
...
/*						case SYSNEWGRP:		*/
		...
		case TFORK:
			...
			else	/* this is the forked branch (child) of execute */
			{
				...
				if (type != TCOM)
				{
					execute(forkptr(t)->forktre, exec_link | 01, errorflg, 0, 0);
				}
		...
		case TPAR:
			execute(parptr(t)->partre, exec_link, errorflg, 0, 0);
			done(0);
		...
		case TLST:
			execute(lstptr(t)->lstlef, 0, errorflg, 0, 0);
			execute(lstptr(t)->lstrit, exec_link, errorflg, 0, 0);
			break;

		case TAND:
			if (execute(lstptr(t)->lstlef, 0, 0, 0, 0) == 0)
				execute(lstptr(t)->lstrit, exec_link, errorflg, 0, 0);
			break;
		...
		case TFOR:
			{
				...
				{
					assign(n, *args++);
					execute(forptr(t)->fortre, 0, errorflg, 0, 0);
					if (breakcnt < 0)
						execbrk = (++breakcnt != 0);
				}
				if (breakcnt > 0)
		...
		case TUN:
			{
				...
				loopcnt++;
				while (execbrk == 0 && (execute(whptr(t)->whtre, 0, 0, 0, 0) == 0) == (type == TWH))
				{
					if(flags & noexec)
						break;
					i = execute(whptr(t)->dotre, 0, errorflg, 0, 0);
					if (breakcnt < 0)
						execbrk = (++breakcnt != 0);
				}
				if (breakcnt > 0)
		...
		case TIF:
			if (execute(ifptr(t)->iftre, 0, 0, 0, 0) == 0)
				execute(ifptr(t)->thtre, exec_link, errorflg, 0, 0);
			else if (ifptr(t)->eltre)
				execute(ifptr(t)->eltre, exec_link, errorflg, 0, 0);
			else
		...
		case TSW:
			{
				...
				while (regp)
				{
					...
					while (rex)
					{
						...
						{
							execute(regp->regcom, 0, errorflg, 0, 0);
							regp = 0;
							break;
						}
						else
...
execexp(char *s, void *f)
{
	...
	if (s)
	{
		...
		initf((int)(long)f);
	execute(cmd(NL, NLFLG | MTFLG), 0, (int)(flags & errflg), 0, 0);
	pop();
}
